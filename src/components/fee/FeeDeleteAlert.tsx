    ...
    >
      ...
      >
        <div className="p-6">
          ...
          <h2 className="text-xl font-display font-bold text-on-surface text-center mb-2">Hapus Biaya</h2>
          <p className="text-on-surface-variant text-body-md text-center">
            Apakah Anda yakin ingin menghapus <span className="font-bold text-on-surface">{selectedFee.fee_name}</span>? Tindakan ini tidak dapat dibatalkan.
          </p>
        </div>
        
        <div className="p-4 bg-surface-container-low flex flex-col-reverse sm:flex-row gap-3 justify-end border-t border-outline-variant/10">
          <button
            onClick={closeAlert}
            disabled={isPending}
            className="w-full sm:flex-1 px-4 py-2 font-label-md rounded-lg text-on-surface-variant hover:bg-surface-variant/20 transition-colors disabled:opacity-50"
          >
            Batal
          </button>
          <button
            ...
            disabled={isPending}
            className="w-full sm:flex-1 flex items-center justify-center gap-2 px-4 py-2 font-label-md rounded-lg bg-error text-on-error hover:bg-error/90 transition-colors disabled:opacity-50 active:scale-95"
          >
